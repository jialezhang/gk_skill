...
4. Run focused boundary checks after each merge wave. Reconcile valid evidence, then run the approved full suite/build once on the frozen integrated candidate and only the exact-target scenarios whose claims remain uncovered or invalidated.
...
7. Aggregate Goal session IDs, commits, checkpoint reports, model-routing records, exact-turn token usage by model/stage, verification evidence, and unresolved risks into the program handoff and retrospective. Generate telemetry with `python3 scripts/collect_delivery_telemetry.py <model-routing.jsonl> --require-complete`; merge per-Goal reports without replaying forked parent turns. Mark unavailable telemetry explicitly; never invent it.
