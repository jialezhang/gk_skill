...
**REQUIRED SUB-SKILL:** Read and apply the conflict-preservation rules from `merge-push-primary` before merging any recorded worktree tip.
...
6. Order source branches by known dependencies. Skip a source only when `git merge-base --is-ancestor <source-head> HEAD` succeeds. Merge each remaining recorded HEAD with `git merge --no-ff --no-commit <source-head>`. Apply the conflict-preservation rules from `merge-push-primary` to every merge: inventory both tips, forbid whole-file side selection, preserve or integrate each side's new behavior and tests, and require an explicit user instruction or approved repository requirement for removals. Review the staged diff and `git diff --cached --check`, compare the candidate with both tips, run focused checks, then create a repository-compliant Lore merge commit. Resolve only unambiguous in-scope conflicts; stop on product or ownership conflicts.
