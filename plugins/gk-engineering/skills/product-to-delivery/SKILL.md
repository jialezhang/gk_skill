...
5. If the user chooses `PRD_REQUIRED`, or the request crosses a governed product boundary, invoke `$grill-me` when intent is unclear; otherwise invoke `$create-product-prd`. Prefer `gpt-5.6-sol`; when the live routing surface does not expose Sol or rejects the selection, keep the current model and record `sol_route_fallback` instead of blocking the lifecycle.
...
8. Invoke `$create-implementation-plan`, preferring `gpt-5.6-sol` and otherwise continuing on the current model under `sol_route_fallback`, and use the validated scope decision. A split decision creates one program baseline and 2–3 bounded Goal plans; a single decision creates one Goal plan.
...
Prefer `gpt-5.6-sol` for PRD and implementation-planning work. If Sol is not listed by the live routing surface, selection is rejected, or runtime metadata proves a mismatch, continue with the current model. Append an audited `sol_route_fallback` record with the failure reason and live evidence; never return `MAIN_AGENT_SOL_REQUIRED` or block a Goal solely because Sol is unavailable.

## Authority

- Product decisions belong to the user.
- PRD authorship/revision and implementation-plan authorship/revision prefer Sol; if unavailable, the current model continues under `sol_route_fallback`. Scope judgment and other high-risk architecture decisions follow the same routing rule.
