...
- `plan_conflict`: repository/runtime evidence contradicts a technical baseline or dependency; return it to the current main agent, which revises the affected plan in its Sol planning stage without delegation.
...
Final acceptance uses an independent Terra thread and valid same-candidate evidence. Return `TARGET_VERIFIED` only when every condition holds; escalate to Sol only if reconciliation exposes a product, plan, architecture, or high-risk security contradiction.
